...
- Backend high-density terms must be extracted into atomic prerequisite knowledge cards when they appear across chapters or require industry context. Examples: source of truth, data lifecycle, transaction boundary, schema migration, isolation level, connection pool, timeout, deadline, exponential backoff, jitter, retry storm, thundering herd, transient failure, partial failure, cascading failure, load shedding, token bucket, dependency isolation, bulkhead, fallback, fail fast, retry budget, TTL, eviction, cache aside, hot key, cache stampede, broker, ack/nack, retry policy, consumer lag, dead-letter queue, replay runbook, duplicate delivery, idempotency, outbox, backpressure, rate limit, circuit breaker, failover, log schema, metrics, metric cardinality, trace context, SLI/SLO, alert runbook, authorization, data masking, secret management, TLS/mTLS, audit log, degradation, downtime, readiness, liveness, graceful shutdown.
- Additional Backend knowledge-card candidates from WRAP research include queue operation terms (prefetch, in-flight message, unacked message, redelivery, requeue, poison message, queue depth, publisher confirm, message persistence, consumer group, partition, offset, retention), migration/data-sync terms (online migration, cutover, fallback plan, CDC, replication lag, checkpoint, backfill, dual write, shadow read, correctness check, data completeness, reconciliation), cache terms (hit/miss, hit rate, warmup, prefetching, cold start, write-through, write-behind, stale data, soft TTL, singleflight), observability terms (histogram, bucket, percentile, error budget, burn rate, sampling, correlation ID, request ID, trace ID, span, symptom-based alert, runbook link, alert fatigue), and API security terms (authentication, BOLA/IDOR, BOPLA, mass assignment, excessive data exposure, unrestricted resource consumption, function-level authorization, tenant boundary, least privilege, security misconfiguration, input validation, SSRF, PII, data classification).
- Knowledge cards must explain domain knowhow, not just dictionary definitions. Each card should cover the concept's system responsibility, observable signals, realistic service examples, design responsibility, and operational consequences.
- Knowledge cards must link high-density terms to other cards when the term already has a card. If a term is ambiguous or has no card, either clarify the term inline or add a new atomic card before using it as a knowledge-network node. Example: "channel" in Backend cards should be clarified as in-process channel / local queue abstraction, not assumed to mean a specific language feature.
