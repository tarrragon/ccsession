...
| 指標 | 目標值 |
|------|--------|
| 端到端延遲（寫入到 UI 顯示） | 目標 < 500ms，上限 < 1 秒 |
...
端到端延遲目標 < 500ms 的分配方案（包含 100ms buffer）：

| 層級 | 預算 | 說明 |
|------|------|------|
| File Watcher（UC-006） | <= 100ms | fsnotify 事件偵測 + 檔案讀取 |
| JSONL Parser（UC-007） | <= 50ms | 單行 JSON parse + 事件轉換 |
| Session Manager（UC-008） | <= 20ms | 狀態更新 + 事件路由 |
| WebSocket 推送（UC-009） | <= 30ms | 序列化 + 網路傳輸（localhost） |
| Flutter 事件處理 + 渲染 | <= 200ms | 反序列化 + 狀態更新 + UI rebuild |
| **合計目標** | **<= 400ms** | 上限 500ms（含 100ms buffer） |

**設計考量**：
- Flutter 層（200ms）預算最大，因為涉及 UI rebuild，且用戶對 100-200ms 延遲較不敏感
- Backend 各層（< 200ms）維持低延遲，確保網路往返時間內可完成
- 上限 < 1 秒 保留作為 P1 容錯機制（網路延遲、系統尖峰）

---

## 驗收條件

- [ ] 新事件從 JSONL 寫入到 UI 顯示延遲目標 < 500ms（上限 < 1 秒）
- [ ] 連續快速事件不遺漏
- [ ] 不完整的 JSON 行不導致錯誤
