...
| 認知負擔指數 | > 10 | 必須拆分 |
| 跨架構層級數 | > 2 層 | 必須拆分 |
| 程式碼實作行數 | > 1000 行（含新增和修改） | 必須拆分 |
| 測試檔案數 | > 3 個 | 建議拆分 |
| 預估 context 消耗 | > 20K tokens（含設計文件讀取 + 實作 + 測試） | 必須拆分 |
| 策略文件中的獨立模組數 | > 1 個且無交叉依賴 | 建議拆分 |

**無需拆分**：上述觸發條件均未命中（檔案 <= 5、負擔 <= 10、層級 <= 2、實作行數 <= 1000、測試檔案 <= 3、context <= 20K、單一模組或有交叉），且修改目的單一明確時，由單一代理人執行，無需拆分。
...
**Version**: 2.10.0 - 3b 拆分決策表新增 3 個指標（實作行數/測試檔案數/context 預算）+ Context 預算評估章節（0.1.1-W12-006.1）
