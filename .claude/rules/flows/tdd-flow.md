...
| 指標 | 觸發條件 | 判定 |
|------|---------|------|
| 測試群組數 | Phase 2 定義的 GWT scenario group > 1 個且無交叉依賴 | 建議拆分 |
| 認知負擔指數 | > 10 | 必須拆分 |
| 跨架構層級數 | > 2 層 | 必須拆分 |
| 預估 context 消耗 | > 20K tokens（含規格讀取 + 實作 + 測試） | 必須拆分 |
...
**無需拆分**：上述觸發條件均未命中（單一測試群組、負擔 <= 10、層級 <= 2、context <= 20K），且修改目的單一明確時，由單一代理人執行。
...
**Version**: 2.12.0 - 3b 拆分策略從模組導向改為測試群組導向，拆分單位為 Phase 2 GWT scenario group（0.2.0-W3-004）
