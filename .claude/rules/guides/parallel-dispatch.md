...
> **核心訊息**：派發本身就代表 PM 已驗證權限。如果被派發編輯某路徑，代理人應放心執行，無需預先上報或評估風險。

上方「派發前路徑權限確認」是 **PM 的職責**，不是代理人的。代理人應理解：

| 概念 | 說明 |
|------|------|
| **用戶授權限制** | 用戶可能拒絕特定工具操作（如 Edit .claude/hooks/）。這由 PM 在派發前處理，與代理人無關 |
| **系統技術限制** | Hook 可能阻擋特定路徑。限制類 Hook 已加入 subagent bypass，subagent 不受影響 |
| **派發即授權** | PM 完成路徑權限確認後才派發。收到派發 = 權限已驗證，應立即嘗試執行 |

**代理人行動原則**：
- 被派發 → 直接嘗試 Edit/Write → 如果被阻擋再上報
- **禁止**：未嘗試就放棄、預先評估權限風險、主動上報「可能被阻擋」
