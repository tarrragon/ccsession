...
<!-- 指引：用 GWT 格式描述所有行為場景。目標 5-15 個場景，< 1500 tokens。
     分組方式：
...
<!-- 指引：定義每種錯誤情境的處理策略。目標 < 500 tokens。
     - 列出所有可能的錯誤類型
...
<!-- 指引：列出可直接驗證的條件。目標 5-10 條，< 500 tokens。
     - 每條以 checkbox 格式開頭
...
## Full validate 維度（全部 7 個維度）

Full 模式的 `/spec validate` 掃描全部 7 個維度：

| # | 維度 | 核心問題 | 重點檢查區段 |
|---|------|---------|------------|
| 1 | 邊界完整性 | 極端值、空值、上限下限的行為定義了嗎？ | Scenarios, API Signatures |
| 2 | 錯誤路徑 | 每個操作失敗時，系統如何回應？ | Error Handling, Scenarios |
| 3 | 狀態完整性 | 所有狀態和轉換都定義了嗎？有不可達狀態嗎？ | Scenarios, Purpose |
| 4 | 並發安全 | 多個使用者/執行緒同時操作會怎樣？ | Scenarios, API Signatures |
| 5 | 效能約束 | 資料量增長 10x/100x 時行為如何？ | Acceptance, Purpose |
| 6 | 安全性 | 誰可以執行此操作？敏感資料如何保護？ | API Signatures, Dependencies |
| 7 | 依賴明確性 | 外部依賴的契約是否明確？不可用時的降級？ | Dependencies, Error Handling |
...
預算是指引而非硬性限制。複雜功能可超出，但應反思是否需要拆分 Ticket。
